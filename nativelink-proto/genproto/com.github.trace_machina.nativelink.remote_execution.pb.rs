...
pub struct ConnectWorkerRequest {
    ...
    /// / The NativeLink version of the worker. Used by the scheduler to warn
    /// / when the worker and scheduler versions do not match. May be empty if
    /// / the version is unknown (for example an older worker).
    #[prost(string, tag = "4")]
...
pub struct ConnectionResult {
    ...
    #[prost(string, tag = "1")]
    pub worker_id: ::prost::alloc::string::String,
    /// / The NativeLink version of the scheduler. Used by the worker to warn
    /// / when the worker and scheduler versions do not match. May be empty if
    /// / the version is unknown (for example an older scheduler).
    #[prost(string, tag = "2")]
    pub version: ::prost::alloc::string::String,
}

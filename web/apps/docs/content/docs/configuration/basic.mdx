...
Even a demo config keeps `AC_MAIN_STORE` separate from
`CAS_MAIN_STORE` — [AC and CAS must never share a
store](/reference/nativelink-config/store-overview#common-compositions),
or their keys collide.
...
[Setup](/getting-started/setup#point-your-build-system-at-it)).
